...
- **`MSETEX`, `DELEX` and `INCREX` are marked `wire = "unverified"` on purpose.** The storage half takes pairs, a key, a deadline and a count, and there is no argument order in it to get wrong. The three are recent enough that writing an argument order down from memory would make it look verified because it is in the code, so the file says out loud that it is not, and the flag flips when the dispatch layer checks them against a real 8.10.
...
Three new ones, all in the string type, all listed here rather than left to be discovered. The full register, including the seven that came out of the spec, is `divergences.toml` at the root.
