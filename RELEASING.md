...
The credential is `CARGO_REGISTRY_TOKEN` on the `crates.io` environment, scoped to publish-new and publish-update. It is on an environment and not in the repository secrets so that exactly one job can see it and so the audit log has somebody's name against each use. The environment's deployment policy allows `v*` tags and `main`, and nothing else, so no other branch can reach the token even if a workflow on that branch asks for it.

`main` is on that list on purpose, and the reason is the one thing about release workflows that catches everybody out once. A tag push runs the copy of `release.yml` that is in the tag. A fix landed on `main` after a tag was cut never applies to that tag, and the tag cannot be moved to pick it up, so dispatching a re-run from the tag reruns the same broken workflow and fails the same way. v0.3.5 is the worked example: it was tagged before the publish became a loop that waits out the rate limit, its own copy ran a plain `cargo publish --workspace`, and it stopped on the first refusal with one crate of eighteen uploaded. The recovery is to dispatch `release.yml` from `main` with `tag:` set, which gives you main's workflow against the tag's code, because every checkout in the file takes `ref: ${{ inputs.tag || github.ref }}`. That only works if `main` can deploy to this environment, which is why it is allowed, and a run refused by the branch policy is easy to misread because the job never starts and so has no steps and no log at all.
...
crates.io rate limits a brand new crate name much harder than a new version of a name that already exists, roughly one every ten minutes against one a minute, and the first release of this workspace was eighteen new names at once. `cargo publish --workspace` stops on the first refusal, so the publish step is a loop: each pass asks the registry what is still missing, publishes what it can, and when it is turned away it reads the time crates.io named in the refusal and sleeps until then. Progress is measured against the registry rather than against what cargo thinks it did, so a pass that uploads three and is then refused is three crates of progress. That is why the job's timeout is five and a half hours rather than thirty minutes, and it costs nothing from the second release onwards, when no name is new and the first pass finishes.
